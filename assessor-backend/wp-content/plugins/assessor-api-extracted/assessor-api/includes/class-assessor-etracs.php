...
class Assessor_Etracs {
    ...
    private function create_real_property($params) {
        ...
        $data = array(
            ...
            'cadastral_lot_no' => sanitize_text_field($params['cadastral_lot_no'] ?? $params['lot_number'] ?? ''),
            'survey_no'        => sanitize_text_field($params['survey_no'] ?? ''),
            'block_no'         => sanitize_text_field($params['block_no'] ?? ''),
    ...
    private function update_real_property($id, $params) {
        ...
        $fields = array('pin', 'cadastral_lot_no', 'survey_no', 'block_no', 'barangay', 'municipality', 'province', 'north', 'south', 'east', 'west');
        $update = array();
        foreach ($fields as $f) {
            // Map alternative field names
            $param_key = $f;
            if ($f === 'cadastral_lot_no' && !isset($params[$f]) && isset($params['lot_number'])) $param_key = 'lot_number';
